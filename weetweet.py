...
desc_dict = dict(
    user="<user>[<id><count>|<id>|<count>], Request user timeline, "
        ...
        "<count> is how many tweets to get, valid number is 1-{0}".format(MAX_REQUEST_LIMIT),
    ...
    create_stream="Create a twitter stream with the following filter "
        ...
        "stream.",
    restart_home_stream="Restart the home timeline stream after it has shutdown.",
)
    ...
    for message in tweet_data:
        if message.get('retweeted_status'):
            if alt_rt_style:
                ...
                message['text'] = message['retweeted_status']['text'] + " (retweeted by " + message['user'][
                    'screen_name'] + ")"
...
def stream_message(buffer, tweet):
    if 'delete' in tweet:
        ...
        weechat.prnt(buffer, "{0}recv stream data: {1}".format(weechat.prefix("network"), str(tweet)))
...
def twitter_stream(cmd_args):
    ...
    try:
        if cmd_args[-1][0] == "{":  # TODO: what if it isn't?
...
def create_stream(name, args=""):
    ...
    if not sock_fd_dict.get(name):  # TODO: what if it isn't?
...
def my_process_cb(data, command, rc, out, err):
    ...
    if out != "":
        if out[0] != "[" and out[0] != "{":
            ...
            weechat.prnt(buffer, "{0}{1}".format(weechat.prefix("network"), out))
            process_output = []
        else:
        ...
        if len(end_mes) >= 1 and end_mes[0] == "L":
            ...
            weechat.prnt_date_tags(
                buffer, 0, "no_highlight",
                "{0}{1}: {2}{3}".format(t_id, end_mes[1:], process_output, more)
            )
            return weechat.WEECHAT_RC_OK

        if end_mes == "About":
            weechat.prnt(buffer, "Nick: {0} | Name: {1} | Protected: {2}".format(
                process_output['screen_name'],
                process_output['name'],
                process_output['protected']
            ))
            weechat.prnt(buffer, "Description: {0}".format(
                process_output['description']
            ))
            weechat.prnt(buffer, "Location: {0} | Time zone: {1}".format(
                process_output['location'],
                process_output['time_zone']
            ))
            weechat.prnt(buffer, "Created at: {0} | Verified user: {1}".format(
                process_output['created_at'],
                process_output['verified']
            ))
            weechat.prnt(buffer, "Following: {0} | Followers: {1} | Favourites: {2} | Tweets: {3}".format(
                process_output['friends_count'],
                process_output['followers_count'],
                process_output['favourites_count'],
                process_output['statuses_count']
            ))
            weechat.prnt(buffer, "You are {0}currently following this person".format(
                "not " * (not process_output['following'])
            ))
            return weechat.WEECHAT_RC_OK
        ...
        if end_mes != "id" and end_mes != "":
            weechat.prnt(buffer, "{0}{1}".format(weechat.prefix("network"), end_mes))
    if err != "":
        weechat.prnt("", "stderr: {0}".format(err))
    return weechat.WEECHAT_RC_OK
    ...
    try:
        ...
        if cmd_args[3] == "settings":
            ...
            if len(cmd_args) == 7:
                ...
                if int(cmd_args[5]) <= MAX_REQUEST_LIMIT:
            ...
            if len(cmd_args) == 6:
                ...
                if int(cmd_args[4]) <= MAX_REQUEST_LIMIT:
            ...
            return "".join(
                "{0}:\n"  # header for main resource
                "{1}".format(
                    resource_name,
                    "".join(
                        "  {0}:\n"  # header for subresource
                        "    reset: {1}\n"
                        "    limit: {2}\n"
                        "    remaining: {3}\n".format(
                            subresource_name,
                            time.strftime(
                                '%Y-%m-%d %H:%M:%S',
                                time.localtime(subresource['reset'])
                            ),
                            str(subresource['limit']),
                            str(subresource['remaining'])
                        )
                        for subresource_name, subresource in resource.items()
                    )
                )
                for resource_name, resource in tweet_data['resources'].items()
            )
            # TODO: delete this nightmare
            # output = ""
        ...
        if "errors" in data and "message" in data["errors"][0]:
            ...
            return "Unexpected error in get_twitter_data:{0}\n Call: {1}".format(sys.exc_info(), cmd_args[3])
    except:
        return "Unexpected error in get_twitter_data:{0}\n Call: {1}".format(sys.exc_info(), cmd_args[3])
...
def buffer_input_cb(data, buffer, input_data):
    ...
    if input_data[0] == ':':
        ...
        if command == 'd' and tweet_dict.get(input_args[1]):
            ...
            weechat.prnt(buffer, "{0}You deleted the following tweet:".format(weechat.prefix("network")))
            ...
            weechat.prnt(buffer, "{0}You fave'd the following tweet:".format(weechat.prefix('network')))
        elif command == 'unfav' and tweet_dict.get(input_args[1]):
            input_data = 'unfav ' + tweet_dict[input_args[1]]
            weechat.prnt(buffer, "{0}You unfave'd the following tweet:".format(weechat.prefix('network')))
        elif command == 'fart' and tweet_dict.get(input_args[1]):
            end_message = "id"
            input_data = 'fart ' + tweet_dict[input_args[1]]
            weechat.prnt(buffer, "{0}You fave'd and retweeted the following tweet:".format(weechat.prefix('network')))
        elif command == 'cnicks':
    ...
    weechat.hook_process("python3 " + SCRIPT_FILE_PATH + " " +
                         ...
                         input_data + " " + '"' + str(options) + '"', 10 * 1000, "my_process_cb",
                         str([buffer, end_message]))
    return weechat.WEECHAT_RC_OK
...
def oauth_proc_cb(data, command, rc, out, err):
    ...
    if len(out) > 16 and out[:16] == "Unexpected error":
        ...
        weechat.prnt(
            buffer,
            "An error occured, check root buffer. Most likely a connection problem, "
            "fix it and the reload weetweet"
        )
        return weechat.WEECHAT_RC_OK
...
def main():
    ...
    weechat.prnt("", "weechat version is " + repr(weechat.info_get('version', "")) + ", numeric " + repr(
